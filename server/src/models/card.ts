...
class Card {
  ...
  static async update(card: CardDTO.UPDATE) {
    try {
      const date = Math.floor(Date.now()/ 1000);
      const cardData = await mysql.connect((con: any) => {
        return con.query(`UPDATE card SET order_weight = '${card.order_weight}', card_title = '${card.card_title}', last_update = '${date}', content = '${card.content}' WHERE card_id = '${card.card_id}'`)
      });
      return card;
    } catch (err) {
      throw err;
  ...
  static async delete(card: CardDTO.UPDATE) {
    try {
      const date = Math.floor(Date.now()/ 1000);
      const cardData = await mysql.connect((con: any) => {
        return con.query(`UPDATE card SET removed = '${1}', last_update = '${date}' WHERE card_id = '${card.card_id}'`)
      });
      return card;
    } catch (err) {
      throw err;
